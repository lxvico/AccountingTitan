...
jQuery(document).ready(function ($) {
  ...
  $(".wf301_reset_hits").on("click", function (e) {
    e.preventDefault();
    button = $(this);
          ...
          if (typeof result.value != "undefined") {
            ...
            $.post({
              ...
              data: {
                action: "wf301_run_tool",
                _ajax_nonce: wf301_vars.run_tool_nonce,
                tool: "delete_log_group",
                log: $(button).data("delete-log"),
                group: $(button).data("delete-group"),
                value: $(button).data("delete-group-value"),
              },
            })
              .always(function (response) {
                wf301_swal.close();
  ...
  $("#wf301-redirects-table").one("preInit.dt", function () {
    $("#wf301-redirects-table_filter").append(
      '<a class="button button-primary button-green add-redirect-rule">Add New Redirect Rule <i class="wf301-icon wf301-redirect"></i></a><br/><div id="tag_area" style="display: none"><label for="selected-tags">Tags:</label><input type="text" id="selected-tags"></div>'
    );
  ...
  $(".settings_page_301redirects").on(
    ...
    function (e) {
      ...
      if(toggle.hasClass('tooltipstered')){
    ...
    function (e) {
      ...
      $.get(
        ...
        function (response) {
          wf301_swal.close();
          if (
            300 < parseInt(response.data.status) &&
            parseInt(response.data.status) < 310
          ) {
          ...
          if (response.data.url) {
            check_html += "<strong>URL:</strong> " + response.data.url + "<br />";
          }
          if (response.data.redirect_message) {
            check_html +=
              "<strong>Result:</strong> " + response.data.redirect_message + "<br />";
          }
